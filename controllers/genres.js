...
function list(req, res, next) {
    Genre.findAll().then(objects => res.json(objects)).catch(err => res.send(err));
}
...
function replace(req, res,next){
    ...
    Genre.findByPk(id).
        then(object=>{
            ...
            object.update({description: description, status: status}).then(obj => res.json(obj)).catch(err=>res.send(err));
    }).catch();
}
